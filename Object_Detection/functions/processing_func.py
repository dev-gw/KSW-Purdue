...
def visualize_feature(feature):
    ...
    test_path = '../Dataset/trim_test/6.wav'
    y_test, sr_test = librosa.load(test_path, sr=22050)
    y_uav, sr_uav = librosa.load(uav_path, sr=22050)
    y_noise, sr_noise = librosa.load(noise_path, sr=22050)
    
    # MFCC
    if feature == 'mfcc':
        mfcc_uav = librosa.feature.mfcc(y_test, sr_test, n_mfcc=40)
    ...
    # Chroma_stft
        ...
        ax1 = plt.subplot(2,1,1)
        librosa.display.specshow(chroma_uav, x_axis='time', y_axis='chroma')
        plt.title('UAV')
        ax2 = plt.subplot(2,1,2, sharex=ax1)
        librosa.display.specshow(chroma_noise, x_axis='time', y_axis='chroma')
        plt.title('Noise')
        plt.tight_layout()
...
if __name__ == '__main__':
    ...
    #visualize_only_mfcc('MFCC','../Dataset/DJI_Phantom4/5.wav' )
